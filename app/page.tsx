...
export default function HomePage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (
        ...
        Array.isArray((reportsResponse.data as { reports?: Report[] }).reports)
      ) {
        setReports(
          (reportsResponse.data as { reports?: Report[] }).reports || []
        );
      }
      ...
      if (isAuthenticated && user?.is_admin) {
        ...
        const userReportsResponse = await apiService.getReports({
          user_only: true,
          limit: 100,
        });
        if (userReportsResponse.data) {
          const userReports =
            (userReportsResponse.data as { reports?: Report[] }).reports || [];
          setStats({
            total_reports: userReports.length,
            pending_reports: userReports.filter(
              (r: Report) => r.status === "Pending"
            ).length,
            in_progress_reports: userReports.filter(
              (r: Report) => r.status === "In Progress"
            ).length,
            resolved_reports: userReports.filter(
        ...
        if (allReportsResponse.data) {
          const allReports =
            (allReportsResponse.data as { reports?: Report[] }).reports || [];
          setStats({
            total_reports: allReports.length,
            pending_reports: allReports.filter(
              (r: Report) => r.status === "Pending"
            ).length,
            in_progress_reports: allReports.filter(
              (r: Report) => r.status === "In Progress"
            ).length,
            resolved_reports: allReports.filter(
              (r: Report) => r.status === "Resolved"
            ).length,
          });
        }
  ...
  // make ugent status color function
  const handleMarkUrgent = async (reportId: string) => {
    try {
      const response = await fetch(`http://localhost:8000/api/reports/${reportId}/urgency/`, {
  method: "POST",
});

      if (!response.ok) {
        throw new Error("Failed to mark urgent");
      }

      // Update local state to show updated urgency count immediately
      setReports((prev) =>
        prev.map((r) =>
          r.id === reportId
            ? { ...r, urgency_count: (r.urgency_count || 0) + 1 }
            : r
        )
      );
    } catch (error) {
    ...
    <div className="min-h-screen">
        ...
        <div className="relative max-w-6xl mx-auto text-center">
          <div className="animate-fade-in-up">
            ...
            <p className="text-xl text-gray-600 mb-8 max-w-3xl mx-auto font-roboto leading-relaxed">
              Join thousands of citizens in making our city cleaner. Report
              waste issues, track their resolution, and be part of the
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href={isAuthenticated ? "/report" : "/login"}>
                <Button
                  size="lg"
                  className="eco-button-primary text-lg px-8 py-4"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="eco-card p-8 text-center group hover:shadow-eco-lg transition-all duration-300">
              ...
              <p className="text-gray-600 font-roboto">
                Spot a waste problem? Take a photo and report it with your
                location.
              </p>
            </div>
            <div className="eco-card p-8 text-center group hover:shadow-eco-lg transition-all duration-300">
              ...
              <p className="text-gray-600 font-roboto">
                Monitor the status of your reports and see real-time updates.
              </p>
            </div>
            <div className="eco-card p-8 text-center group hover:shadow-eco-lg transition-all duration-300">
              ...
              <p className="text-gray-600 font-roboto">
                Watch as your community becomes cleaner through collective
                action.
              </p>
            </div>
          </div>
          ...
          {dataLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[...Array(4)].map((_, i) => (
                <Card
                  key={i}
                  className="eco-card text-center p-6 animate-pulse"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <Card className="eco-card text-center p-6 hover:shadow-eco-lg transition-all duration-300">
                <CardHeader className="pb-2">
                  ...
                  <CardTitle className="text-2xl font-poppins text-eco-red">
              ...
              <Card className="eco-card text-center p-6 hover:shadow-eco-lg transition-all duration-300">
                <CardHeader className="pb-2">
                  ...
                  <CardTitle className="text-2xl font-poppins text-eco-amber">
              ...
              <Card className="eco-card text-center p-6 hover:shadow-eco-lg transition-all duration-300">
                <CardHeader className="pb-2">
                  ...
                  <CardTitle className="text-2xl font-poppins text-eco-green">
          ...
          {dataLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (
                <Card
                  key={i}
                  className="eco-card overflow-hidden animate-pulse"
                >
                    ...
                    <div className="absolute top-3 right-3">
                      <Badge
                        className={`${getStatusColor(
                          report.status
                        )} font-roboto font-medium px-3 py-1 rounded-full`}
                      >
                        {report.status}
                      </Badge>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 text-sm text-gray-500 font-roboto">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          className="text-xs px-3 py-1 border-eco-red text-eco-red hover:bg-eco-red hover:text-white"
                          onClick={() => handleMarkUrgent(report.id)}
                        >
                          Mark Urgent
                        </Button>
                      </div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-eco-teal to-eco-green relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto text-center text-white">
          <h3 className="text-4xl font-poppins font-bold mb-6">
            Ready to Make a Difference?
          </h3>
          <p className="text-xl mb-8 font-roboto opacity-90">
            Join our community of environmental champions and help create a
            cleaner, greener city for everyone.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div className="col-span-1 md:col-span-2">
              ...
              <p className="text-gray-300 font-roboto leading-relaxed">
                ...
                through collaborative waste reporting and tracking.
              </p>
            </div>
            <div>
